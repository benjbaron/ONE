...
public class ExternalMovement extends MovementModel {
	...
	private static List<Triplet<String, Coord, String>> initLocations;
	...
	private ExternalMovement(MovementModel mm) {
		...
		if (initLocations.size() > 0) { // we have location data left
			...
			Triplet<String, Coord, String> initLoc = initLocations.remove(0);
			this.intialLocation = this.latestLocation = initLoc.getB();
			this.latestPathStartTime = initTime;
			
			// puts the new model to model map for later updates
			idMapping.put(initLoc.getA(), this);

			String state = initLoc.getC();
			if (state.equals("UP")) this.isActive = true;
			else if (state.equals("DOWN")) this.isActive = false;
		}
		else {
	 ...
	 */
	private void addLocation(Coord loc, double time, String state) {
	...
	private static double readMorePaths() {
		List<Triplet<String, Coord, String>> list = reader.readNextMovements();
		...
		for (Triplet<String, Coord, String> t : list) {
			ExternalMovement em = idMapping.get(t.getA());
			if (em != null) { // skip unknown IDs, i.e. IDs not mentioned in...
				// ...init phase or if there are more IDs than nodes
				em.addLocation(t.getB(), time, t.getC());
			}
		}
